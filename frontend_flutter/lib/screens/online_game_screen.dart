...
/// 입력 모드 상태 (Sealed Class)
sealed class OnlineInputMode {
  const OnlineInputMode();
}

class MovingMode extends OnlineInputMode {
  const MovingMode();
}

class PlacingWallMode extends OnlineInputMode {
...
class _OnlineGameScreenState extends State<OnlineGameScreen> {
  ...
  String _message = '';
  OnlineInputMode _inputMode = const MovingMode();
  ...
  void _movePawn(int row, int col) {
    // 패턴 매칭(is)을 통해 타입 확인
    if (!_canPlay || _inputMode is! MovingMode) return;
  ...
  void _placeWall(int row, int col, String orientation) { // 인자의 orientation은 무시하고 상태에서 가져올 수도 있음
    final currentMode = _inputMode; // 스마트 캐스팅을 위해 로컬 변수 할당
    
    if (!_canPlay || currentMode is! PlacingWallMode) return;

    debugPrint('[OnlineGame] 벽 설치 요청: ($row, $col, ${currentMode.orientation})');
    setState(() {
      _isLoading = true;
      _message = '벽 설치 중...';
    });

    widget.wsService.placeWall(row, col, currentMode.orientation);
  }
      ...
      child: Column(
        children: [
          ...
          UnifiedBoardWidget(
            ...
            // 현재 모드가 PlacingWallMode이면 벽 모드 활성화
            wallMode: _inputMode is PlacingWallMode,

            // 현재 모드가 PlacingWallMode일 때만 방향 정보 전달 (아니면 기본값)
            wallOrientation: _inputMode is PlacingWallMode 
                ? (_inputMode as PlacingWallMode).orientation 
                : 'horizontal',

            onCellTap: _canPlay && _inputMode is MovingMode ? _movePawn : null,
                
            // 벽 설치 콜백
            onWallTap: _canPlay && _inputMode is PlacingWallMode
                ? (row, col, _) => _placeWall(row, col, (_inputMode as PlacingWallMode).orientation)
                : null,
  ...
  Widget _buildMyControls(ColorScheme colorScheme) {
    ...
    final isMovingMode = currentMode is MovingMode;
            ...
            Row(
              ...
              children: [
                ...
                FilledButton.tonal(
                  onPressed: () {
                    setState(() {
                      // [로직] MovingMode <-> PlacingWallMode 전환
                      if (isWallMode) {
                        _inputMode = const MovingMode();
                        debugPrint('[OnlineGame] 모드 변경: 이동 모드');
                      } else {
                        // 벽 모드로 전환 시 기본값은 수평(horizontal)
                        _inputMode = const PlacingWallMode(orientation: 'horizontal');
                        debugPrint('[OnlineGame] 모드 변경: 벽 모드');
                      }
                    });
                  },
                  style: FilledButton.styleFrom(
                    backgroundColor: isWallMode // _wallMode 대신 isWallMode 사용
                  ...
                  child: Row(
                    ...
                    children: [
                      Icon(
                        isWallMode ? Icons.fence : Icons.directions_walk,
                        size: 18,
                      ),
                      const SizedBox(width: 8),
                      Text(isWallMode ? '벽 모드' : '이동 모드'),
                    ],
                  ),
                ),

                // 2. 방향 전환 버튼 (벽 모드일 때만 표시)
                if (isWallMode) ...[
                  ...
                  SegmentedButton<String>(
                    ...
                    // [변경] 현재 상태에서 방향을 가져와 선택 상태 표시
                    selected: {currentMode.orientation},
                    
                    onSelectionChanged: (Set<String> selection) {
                      debugPrint('[OnlineGame] 벽 방향 변경: ${selection.first}');
                      setState(() {
                        // [로직] 같은 PlacingWallMode지만 방향만 다른 새 인스턴스로 교체
                        _inputMode = PlacingWallMode(orientation: selection.first);
                      });
                    },
                  ),
                ],
